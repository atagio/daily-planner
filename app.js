...
function formatTime12(h, m) {
    const h12 = h % 12 || 12;
    const p = h < 12 ? 'ق.ظ' : 'ب.ظ';
    return `${h12}:${m.toString().padStart(2, '0')} ${p}`;
}

function formatTimeRange(sh, sm, eh, em) {
    return `${formatTime12(sh, sm)} - ${formatTime12(eh, em)}`;
}
...
// ---------- پومودورو ----------
let pomodoroInterval = null, pomodoroTimeLeft = 0, pomodoroPhase = 'idle';
let pomodoroSettings = { work: 25, break: 5, parts: 4 }, pomodoroCurrentPart = 0;
...
function setupEvents() {
    ...
    // مودال ورود نام (فقط با استایل اینلاین)
    document.getElementById('saveNameBtn').addEventListener('click', () => {
        ...
        if (!name) {
            ...
            nameInput.focus();
            return;  // مودال همچنان نمایش داده می‌شود
        }

        errorEl.textContent = '';
        userName = name;
        localStorage.setItem('planner_userName', name);
        hideWelcomeModal();  // مخفی کردن قطعی
        updateUIWithName();
    });
...
function init() {
    ...
    // مودال فقط در صورت نداشتن نام نمایش داده شود
    if (!userName) {
        showWelcomeModal();
    } else {
        hideWelcomeModal();
